...
Code Description: A python code to tune the hyperparameters of ChaosNet on the AR dataset.
"""
import os
import numpy as np
...
# Validation
FOLD_NO = 5
INITIAL_NEURAL_ACTIVITY = np.arange(0.01, 0.49, 0.01)
DISCRIMINATION_THRESHOLD = np.arange(0.01, 0.49, 0.01) 
...
# CFEX.transform(X_train_norm, INA, 10000, EPSILON_1, DT)
CFEX_ar_dataset = CFEX.transform(ar_dataset_scaled, BESTINA[0], 10000, BESTEPS[0], BESTDT[0])[:,0:length]
CFEX_random_dataset = CFEX.transform(random_dataset_scaled, BESTINA[0], 10000, BESTEPS[0], BESTDT[0])[:,0:length]
...
RESULT_PATH = PATH + f'/CFEX-TUNING/AR_Hyperparameters/Length_{length}'
