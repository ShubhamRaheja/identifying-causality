...
Code Description: A python code to tune the hyperparameters of LR on the AR+CFEX dataset.
"""
import os
import numpy as np
import pickle
...
DISCRIMINATION_THRESHOLD = [0.38]
EPSILON = [0.181] #np.arange(0.1, 0.49, 0.1)
BESTF1, BESTINA, BESTDT, BESTEPS, BESTMAXITER, BESTC, BESTTOL = k_cross_validation(FOLD_NO, X_train_val, y_train_val, X_test_val, y_test_val, INITIAL_NEURAL_ACTIVITY, DISCRIMINATION_THRESHOLD, EPSILON)
...
# Reload the datasets to build the LR model with best hyperparameters
...
RESULT_PATH = PATH + f'/LR-TUNING/AR_CFEX_Hyperparameters/Length_{length}'
RESULT_PATH_FINAL = PATH + f'/TESTING-RESULTS/On_Training_set/AR_CFEX_Results/Length_{length}'
RESULT_PATH_LR = PATH + '/LR-logs/AR_CFEX_Checkpoints/'
