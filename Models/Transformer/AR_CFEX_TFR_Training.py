...
Code Description: A python code to tune the hyperparameters of TFR on the AR+CFEX dataset.
"""
import os
import numpy as np
...
# Validation
FOLD_NO = 5
INITIAL_NEURAL_ACTIVITY = [0.03] #np.arange(0.01, 0.5, 0.01) #[0.21]
...
# Reload the datasets to build the Transformer encoder model with best hyperparameters
...
# Extract CFX features with best hyperparameters
...
# Reshaping as tensor for Transformer encoder model.
X_train = np.reshape(X_train ,(X_train.shape[0], X_train.shape[1], 1)) 
X_test = np.reshape(X_test,(X_test.shape[0], X_test.shape[1], 1))

# Checkpoints path 
PATH = os.path.dirname(__file__)
RESULT_PATH_TFR = PATH + '/TFR-logs/AR_CFEX_Checkpoints/'
...
RESULT_PATH = PATH + f'/TFR-TUNING/AR_CFEX_Hyperparameters/Length_{length}'
