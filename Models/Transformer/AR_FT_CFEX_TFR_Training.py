...
Code Description: A python code to tune the hyperparameters of TFR on the AR+FT+CFEX dataset.
"""
import os
import numpy as np
...
# Validation
FOLD_NO = 5
INITIAL_NEURAL_ACTIVITY = [0.02] #np.arange(0.01, 0.5, 0.01) #[0.21]
DISCRIMINATION_THRESHOLD = [0.01]
EPSILON = [0.161] # np.arange(0.1, 0.49, 0.1)
...
# Reload the datasets to build the Tranformer encoder model with best hyperparameters
...
RESULT_PATH_TFR = PATH + '/TFR-logs/AR_FT_CFEX_Checkpoints/'
...
# model - Transformer encoder / Building the model
...
RESULT_PATH = PATH + f'/TFR-TUNING/AR_FT_CFEX_Hyperparameters/Length_{length}'
