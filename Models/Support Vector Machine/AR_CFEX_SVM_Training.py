...
Code Description: A python code to tune the hyperparameters of SVM on the AR+CFEX dataset.
"""
import os
import numpy as np
import pickle
...
ar_dataset_scaled = scale_dataset(ar_dataset, ar_dataset.shape[0], ar_dataset.shape[1])
random_dataset_scaled = scale_dataset(random_dataset, random_dataset.shape[0], random_dataset.shape[1])
...
X_val = np.concatenate((random_dataset_scaled, ar_dataset_scaled))
...
# Validation
FOLD_NO = 5
INITIAL_NEURAL_ACTIVITY = [0.31]
DISCRIMINATION_THRESHOLD = [0.47]
EPSILON = [0.181] #np.arange(0.1, 0.49, 0.1)
BESTF1, BESTINA, BESTDT, BESTEPS, BESTC = k_cross_validation(FOLD_NO, X_train_val, y_train_val, X_test_val, y_test_val, INITIAL_NEURAL_ACTIVITY, DISCRIMINATION_THRESHOLD, EPSILON)
...
# Reload the datasets to build the SVM model with best hyperparameters
...
RESULT_PATH = PATH + f'/SVM-TUNING/AR_CFEX_Hyperparameters/Length_{length}'
RESULT_PATH_FINAL = PATH + f'/TESTING-RESULTS/On_Training_set/AR_CFEX_Results/Length_{length}'
RESULT_PATH_SVM = PATH + '/SVM-logs/AR_CFEX_Checkpoints/'
